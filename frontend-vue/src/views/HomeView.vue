...
<template>
  <div class="home">
    <ResponsiveImage class="home-backdrop" :src="coverImageUrl()" alt="西行之路原版封面" sizes="100vw" object-fit="cover" critical />
...
import { EMOJI_MAP, coverImageUrl } from '@/constants/images'
...
.home { min-height: 100dvh; height: auto; padding: 100px 16px 40px; isolation: isolate; background: #100e17; }
.home-backdrop { position: absolute; inset: 0; width: 100%; height: 100%; z-index: 0; }
.home-content { min-height: calc(100dvh - 140px); justify-content: center; width: min(100%, 680px); }
.identity-bar { max-width: calc(100% - 28px); border-radius: 16px; }
.identity-bar span { overflow-wrap: anywhere; min-width: 0; }
.identity-bar button { min-height: 44px; }
.subtitle { line-height: 1.6; }
.menu-btn { min-height: 80px; }
.save-slot { grid-template-columns: 1fr; }
@media (max-width: 600px) {
  .menu-grid { width: 100%; }
  .identity-bar { right: 14px; top: 14px; }
  .title { font-size: clamp(32px, 8vw, 42px); }
}
