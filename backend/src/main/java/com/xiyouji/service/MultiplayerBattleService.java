 ...
 *   - 使用 ReentrantLock 保证线程安全，同时只有一张卡牌被结算
...
public class MultiplayerBattleService {
    ...
    /** 每个房间的出牌锁，保证抢出牌的原子性（单实例有效；多实例需用Redis分布式锁） */
    ...
    /**
     * 出牌（抢出牌机制：谁先请求谁先出）
     * 使用 ReentrantLock 保证同一房间同时只有一张卡牌被结算。
     *
     * @param roomCode  房间码
     * @param userId    玩家ID
    ...
    public MultiplayerBattleState playCard(String roomCode, String userId, int handIndex) {
        ReentrantLock lock = getLock(roomCode);
        lock.lock();
        try {
    ...
    /** 获取或创建房间锁 */
    private ReentrantLock getLock(String roomCode) {
    ...
    public Map<String, Object> returnToMap(String roomCode, String requesterId) {
        ...
        try {
            ...
            if (currentNode != null && GameConstants.NODE_BOSS.equals(currentNode.getType())) {
                ...
                result.put("nextLayer", false);
                result.put("message", "返回地图探索");
            }
            ...
            broadcaster.broadcastSystemMessage(roomCode, result.get("message").toString());

            log.info("Return to map: room={}, nextLayer={}", roomCode, result.get("nextLayer"));
            return result;
        } finally {
            lock.unlock();
        }
